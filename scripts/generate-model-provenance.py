...
def emit(root: Path) -> str:
    ...
    provenance_sha256 = hashlib.sha256((root / "Config/ModelProvenance/model-provenance.v1.json").read_bytes()).hexdigest()
    lines = ["// Generated by scripts/generate-model-provenance.py; do not edit.", "import Foundation", "", "struct GeneratedWhisperModel: Sendable {", "    let id: String", "    let displayName: String", "    let filename: String", "    let sha256: String", "    let sizeBytes: Int64", "    let license: String?", "    let languages: [String]?", "    let revision: String", "    let source: String", "}", "", "struct GeneratedParakeetManifestEntry: Codable, Hashable, Sendable {", "    let path: String", "    let size: Int64", "    let sha256: String", "    let component: String", "    let role: String", "", "    init(path: String, size: Int64, sha256: String, component: String = \"Unknown\", role: String = \"unknown\") {", "        self.path = path", "        self.size = size", "        self.sha256 = sha256", "        self.component = component", "        self.role = role", "    }", "}", "", "enum GeneratedModelProvenance {", f"    static let whisperRepository = {swift_string(WHISPER_REPOSITORY)}", f"    static let whisperRevision = {swift_string(WHISPER_REVISION)}", f"    static let parakeetRepository = {swift_string(PARAKEET_REPOSITORY)}", f"    static let parakeetRevision = {swift_string(PARAKEET_REVISION)}", f"    static let fluidAudioRevision = {swift_string(FLUID_AUDIO_REVISION)}", "", "    static let whisper: [GeneratedWhisperModel] = ["]
    for x in whisper:
